                ...
                <div class="card">
                    <div class="row g-0">
                        <div class="col-12 col-lg-5 col-xl-3 border-right">
                            ...
                            <a href="#" id="{{$l->id}}" data-id-lekar="{{$l->id_lekar}}" data-id-chat="{{$l->id}}" data-id-pacijent="{{Session::get('ID')}}" onclick="otvoriChatPacijent(this.id); ucitajChat();" class="list-group-item list-group-item-action border-0">
                                <div class="badge bg-success float-right" id="brojPoruka{{$l->id}}">{{$l->brojPoruka}}</div>
                                <div class="d-flex align-items-start">
                                    <img src="{{ asset('img/doctor.png') }}" class="rounded-circle mr-1" alt="{{$l->ime}} {{$l->prezime}}" width="40" height="40">
                        ...
                        <div class="col-12 col-lg-7 col-xl-9 aktivanRazgovor" style="height:auto;" id="konverzacija{{$r->id}}">
                            <input hidden id="id_posiljalac{{$r->id}}" value="{{$r->id_pacijent}}" />
                            <input hidden id="id_primalac{{$r->id}}" value="{{$r->id_lekar}}" />
                            <input hidden id="idChat{{$r->id}}" value="{{$r->id}}" />
                            <input hidden id="ime{{$r->id}}" value="{{$r->ime}} {{$r->prezime}}" />
                            <div class="py-2 px-4 border-bottom d-none d-lg-block">
                                <div class="d-flex align-items-center py-1">
                                    <div class="position-relative">
                                        <img src="http://127.0.0.1:8000/img/doctor.png" class="rounded-circle mr-1" alt="Sharon Lessman" width="40" height="40">
                                    </div>
                                    <div class="flex-grow-1 pl-3" style="margin-left: 15px;">
                                        <strong>{{$r->ime}} {{$r->prezime}}</strong>
                                        <div class="text-muted small"><em id="typing{{$r->id_lekar}}"></em></div>
                                    </div>
                                </div>
                            </div>
                        ...
                        @else
                        <div class="col-12 col-lg-7 col-xl-9" style="display:none; max-height:500px;" id="konverzacija{{$r->id}}">
                            <input hidden id="id_posiljalac" value="{{$r->id_pacijent}}" />
                            <input hidden id="id_primalac" value="{{$r->id_lekar}}" />
